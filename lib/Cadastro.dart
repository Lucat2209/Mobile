...
class _CadastroState extends State<Cadastro> {
  ...
  "password": _controllerSenha.text,    // alterar para "password"
  ...
  "role": _selectedRole,
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        _showErrorDialog(responseData['message'] ?? 'Erro no registro');
        return false;
      }
    } catch (e) {
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  bool sucesso = await registrarUsuario();
                  if (sucesso) {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
...
// Formatter para telefone (formato brasileiro)
class TelefoneInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if (length > 11) text = text.substring(0, 11);
    ...
    if (length >= 1) {
      buffer.write('(');
      buffer.write(text.substring(0, length >= 2 ? 2 : 1));
      buffer.write(') ');
      index = length >= 2 ? 2 : 1;
    }

    if (length >= 7) {
      buffer.write(text.substring(index, index + 5));
      buffer.write('-');
      buffer.write(text.substring(index + 5));
